...
import { getUserEntities } from '@/features/entity';
...
export default async function AppLayout({
  ...
  const entities = await getUserEntities(supabase, user.id);

  if (!entities || entities.length === 0) {
    redirect('/no-entity');
  }

  const initialEntityId = entities[0].entity_id;

  return (
    <AppProviders initialEntityId={initialEntityId}>
